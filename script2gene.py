...
def solve(intervals, point):
       ...
       for i, j in intervals:
            if point >= i and point <= j:
                count += 1
       return count
w = open("densitiesall3.txt","a")
for i in range(1,25,1):  
    op = open("gccoords_chr_new52_all.txt","r")
    ...
    d1 = subprocess.check_output('wc -l intmut_all{}.bed'.format(i),shell = True) 
    d11 = d1.decode().split()[0]
    d55 = int(d5.decode().split()[0])*52
    print("d11-55 "+str(d11)+"\t"+str(d55)+"\n")
    w.write("chr{}".format(i)+"\t"+"average density"+"\t"+str(float(int(d11)/int(d55)))+"\tinterval length"+"\t"+"52"+"\n")
w.close()
